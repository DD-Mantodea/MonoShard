...
namespace MonoShard.Contents.Logic
{
    public class WayFinder
    {
        ...
        public List<Vector2> FindPath()
        {
            ...
            for (int x = 0; x < _room.TileWidth; x++)
            {
                for (int y = 0; y < _room.TileHeight; y++)
                {
            ...
            while (openSet.Count > 0)
            {
                ...
                foreach (Vector2 neighbor in current.Neighbors())
                {
                    ...
                    if (neighbor.X < 0 || neighbor.X >= _room.TileWidth || neighbor.Y < 0 || neighbor.Y >= _room.TileHeight)
                    {
                        continue;
                    }
